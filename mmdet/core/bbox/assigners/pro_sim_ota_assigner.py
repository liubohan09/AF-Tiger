...
class PROSimOTAAssigner(BaseAssigner):
    ...
    def _assign(self,
        ...
        if self.use_vfl:
            ...
            vfl_score = torch.zeros(valid_pred_scores.shape)
            vfl_score[torch.arange(0, vfl_score.shape[0]), gt_vfl_labels] = pairwise_ious.reshape([-1])
            cls_cost = varifocal_loss(
                valid_pred_scores, vfl_score,use_sigmod=False
                ).reshape([num_valid, num_gt])
            gious =bbox_overlaps(valid_decoded_bbox,gt_bboxes,mode='giou')
            iou_cost =1-gious
            cost_matrix = (
            ...
            cls_cost = F.binary_cross_entropy(
                valid_pred_scores, gt_onehot_label,
