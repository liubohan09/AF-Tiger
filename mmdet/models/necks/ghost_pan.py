...
class GhostModule(BaseModule):
    def __init__(
        self, inp, oup, kernel_size=1, ratio=2, dw_size=3, stride=1,norm_cfg=dict(type='BN'), act_cfg=dict(type="ReLU")
    ):
...
class GhostBottleneck(BaseModule):
    ...
    def __init__(
        ...
        if has_se:
            self.se = SELayer(mid_chs, ratio=se_ratio,act_cfg=(dict(type='ReLU'), dict(type='HSigmoid')))
        else:
            self.se = None
        ...
        if in_chs == out_chs and self.stride == 1:
            ...
            self.shortcut = nn.Sequential(
                ...
                ConvModule(in_chs, out_chs, 1, stride=1, padding=0, bias=False,norm_cfg=norm_cfg,act_cfg=None),
            )
