...
class NanoPlusHead(BaseDenseHead,BBoxTestMixin):
    """
    ...
    def loss(self,
        ...
        if teacher_outs is not None:
            teacher_cls_preds=teacher_outs[0]
            teacher_bbox_preds=teacher_outs[1]
            flatten_teacher_cls_preds=[
                teacher_cls_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1,
                                                     self.cls_out_channels)
                for teacher_cls_pred in teacher_cls_preds
            ]
            flatten_teacher_bbox_preds = [
                teacher_bbox_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4 * (self.reg_max + 1))
                for teacher_bbox_pred in teacher_bbox_preds
            ]
            flatten_teacher_cls_preds = torch.cat(flatten_teacher_cls_preds, dim=1)
            flatten_teacher_bbox_preds = torch.cat(flatten_teacher_bbox_preds, dim=1)

            teacher_dis_preds_feature = self.distribution_project(flatten_teacher_bbox_preds)
            teacher_dis_preds = teacher_dis_preds_feature * center_priors[..., 2, None]
            teacher_decoded_bboxes = self._bbox_decode(center_priors[..., :2], teacher_dis_preds)

            batch_assign_res = multi_apply(
                self._get_target_single,
                flatten_teacher_cls_preds.detach(),
                center_priors,
                teacher_decoded_bboxes.detach(),
                gt_bboxes,
                gt_labels,
            )
        ...
        if teacher_outs is not None:
            teacher_loss = self._get_loss_from_assign(
                flatten_teacher_cls_preds, flatten_teacher_bbox_preds, teacher_dis_preds_feature,teacher_decoded_bboxes, batch_assign_res
            )

            for k, v in teacher_loss.items():
                loss_dict["teacher_" + k] = v
        return loss_dict
