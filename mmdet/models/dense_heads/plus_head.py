...
class PlusHead(BaseDenseHead,BBoxTestMixin):
    """
    ...
    def loss(self,
             ...
             img_metas,
             gt_bboxes_ignore=None,
             batch_assign_res=None,
             ):
        """Compute losses.

        """
        num_imgs = len(img_metas)
        soft_labels,soft_targets =soft_target_list
        featmap_sizes = [cls_score.shape[2:] for cls_score in cls_scores]  # [48,24,12,6]
        # get grid cells of one image
        mlvl_priors = self.prior_generator.grid_priors(
            featmap_sizes,
        ...
        flatten_soft_targets= [
            soft_target.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4*(self.reg_max+1))
            for soft_target in soft_targets
        ]
        flatten_soft_labels=[
            soft_label.permute(0, 2, 3, 1).reshape(num_imgs, -1, self.cls_out_channels)
            for soft_label in soft_labels
        ]

        flatten_cls_preds = torch.cat(flatten_cls_preds, dim=1)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
        flatten_soft_targets=torch.cat(flatten_soft_targets,dim=1)
        flatten_soft_labels=torch.cat(flatten_soft_labels,dim=1)

        dis_preds_feature = self.distribution_project(flatten_bbox_preds)
        dis_preds = dis_preds_feature * center_priors[...,2 , None]
        decoded_bboxes = self._bbox_decode(center_priors[..., :2], dis_preds)
        loss_dict = self._get_loss_from_assign(
            flatten_cls_preds,flatten_bbox_preds,dis_preds_feature, decoded_bboxes,flatten_soft_labels,flatten_soft_targets, batch_assign_res
        )
        return loss_dict

    def _get_loss_from_assign(self, cls_preds, reg_preds,dis_preds_feature, decoded_bboxes,flatten_soft_labels,flatten_soft_targets, assign):
        ...
        soft_labels =flatten_soft_labels.reshape(-1,self.num_classes)
        soft_corners=flatten_soft_targets.reshape(-1, 4 * (self.reg_max + 1))
        ...
        if len(pos_inds) > 0:
            ...
            loss_ld = self.loss_ld(
                reg_preds[pos_inds].reshape(-1, self.reg_max + 1),
                soft_corners[pos_inds].reshape(-1, self.reg_max + 1),
                weight=weight_targets[:, None].expand(-1, 4).reshape(-1),
                avg_factor=4.0 * bbox_avg_factor,)
            ...
            loss_ld=reg_preds.sum() * 0
            loss_kd=reg_preds.sum*0

        loss_dict = dict(loss_cls=loss_cls, loss_bbox=loss_bbox, loss_dfl=loss_dfl,loss_ld=loss_ld)
        if self.use_l1:
            if len(pos_inds) > 0:
    ...
    def forward_train(self,
        ...
        losses = self.loss(
            *loss_inputs,
            ...
            batch_assign_res=label_assignment_results)
        return losses
